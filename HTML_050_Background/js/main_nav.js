document.addEventListener("DOMContentLoaded", () => {
  ...
  const navClick = (e) => {
    ...
    if (tagName === "LI") {
      /*
        ...
        left 화면 왼쪽 끝인 0 으로 설정하고
        smooth속성을 부여하여 자연스럽게 화면이 스크롤되도록 설정했다.

        */
      let className = e.target.className;
      // className art1 -> #art1 찾아 선택해라
      let art = document.querySelector("#" + className);

      // art 객체(Dom 객체)의 현재 위치, 모양 값을 getter 해달라
      let bound = art.getBoundingClientRect(); // 값추출
  ...
  엄청나게 많은 event를 발생한다
  이 event 핸들러에서 화면에 무언가 그리는 코드를 
  작성하게 되면 Browser, System에 막대한 자원을 소모한다 
  이러한 불편함을 없애기 위해 임의의 flag 변수를 하나 만들고
  이벤트가 시작되어 화면에 그리는 코드가 시작될때
  ...
  */
  ...
  const scrollTop_nav_tick = (e) => {
    if (!ticking) {
      ...
      // 현재 선택된 tag(art에 담겨 있는 tag)에서 id값을 추출하여 (art1, art2)
      // li 중에 클래스가 id 값과 같은 tag를 선택하라
