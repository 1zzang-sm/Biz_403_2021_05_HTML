...
/* Java와 같은 경우는 if() 문에서 
 Single EQ(=)를 사용하면 systax error가 난다.
 if( num = 3) {}
 하지만 다음과 같은 코드는 논리적으로 문제가 없다.
 if( num = method()) == 3 ) 
 */
// while( (reader = buffer.readeLine() ) != null )
/* while(true)   {
    reader = buffer.readeLine();
...
// js에서는 숫자 0, 문자열"", false, null, undefined, NaN를 제외한
// 모든 것은 boolean에서 true로 취급된다.
// if(값) 명령을 사용할때 값에 위에 나열한 type 데이터가 나타나면
// 논리적으로 false가 인식되고, 그외 무조건 true이다.
...
동등 연산자( == )
    자동으로 형 변환이 발생한다.
    0 == '0' 을 비교하면 문자열 '0'을 숫자 0으로 자동 형변환을 해버린다.
...
// 문자열 "" 과 문자열 "0" 을 비교하므로 결과가 false
console.log("" == "0"); //  false
// 문자열 ""을 숫자 0 으로 변환하고
// 0 == 0 으로 비교하므로 결과가 true
console.log("" == 0); // true
...
// 코드가 다소 복잡해 지지만 강제 형 변환을 하고
// 일치연산자로 비교하는 것이 논리적 오류를 막을 수 있다.
...
// "A" 가 숫자로 바꿀 수 없냐?
console.log(isNaN("A")); // 숫자로 변환 할 수 없어서 true
